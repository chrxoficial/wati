...
const Invoices = () => {
  ...
  const handleOpenLink = async (invoices) => {
    console.log(invoices)
      ...
      // Formatar a data no formato ISO 8601 com fuso horário
      const dataNew = expiresAt.toISOString().replace("Z", "-03:00");

      const newValues = {
        referenceId: invoices.id,
        returnUrl: "https://chat.intalkconnect.dev",
        callbackUrl: "https://chat.intalkconnect.dev",
        value: invoices.value,
        expiresAt: dataNew,
        buyer: {
          document: "09010639754" //cpf
        }
      };

      const headers = {
        "Content-Type": "application/json",
        "x-picpay-token": "61c28a83-fd8b-4ac0-86bd-b73cc4f01236"
      };

      const response = await axios.post(
        "https://appws.picpay.com/ecommerce/public/payments",
        JSON.stringify(newValues), // Passar o corpo como uma string
        {
          headers: headers
        }
      );
    ...
    window.open(data.paymentUrl, "_blank"); // Abrir a URL em uma nova janela do navegador
  };
                  ...
                  <TableCell align="center">
                    {rowStatus(invoices) !== "Pago" ?
                      <Button
                        ...
                        variant="outlined"
                        color="secondary"
                        onClick={() => handleOpenLink(invoices)}
                      >
                        PAGAR
                      </Button> :
                      <Button
